...
impl WawonaCore {
    ...
    pub fn set_force_ssd(&self, enabled: bool) {
        ...
        *self.force_ssd.write().unwrap() = enabled;
        ...
        crate::wlog!(
            crate::util::logging::FFI,
            "Updating {} active decorations",
            decorations_to_configure.len()
        );
    ...
    pub fn stop(&self) -> Result<()> {
        ...
        *compositor_guard = None;
        ...
        // Clear caches
        if let Ok(mut w) = self.ffi_windows.write() {
            w.clear();
        }
        if let Ok(mut s) = self.ffi_surfaces.write() {
            s.clear();
        }
        if let Ok(mut c) = self.ffi_clients.write() {
            c.clear();
        }
        if let Ok(mut t) = self.textures.write() {
            t.clear();
        }
        if let Ok(mut we) = self.pending_window_events.write() {
            we.clear();
        }
        if let Ok(mut ce) = self.pending_client_events.write() {
            ce.clear();
        }
        if let Ok(mut pb) = self.pending_buffers.write() {
            pb.clear();
        }
        if let Ok(mut pr) = self.pending_redraws.write() {
            pr.clear();
        }
    ...
    pub fn is_running(&self) -> bool {
        ...
        guard.as_ref().map(|c| c.is_running()).unwrap_or(false)
    }
        ...
        match event {
            CompositorEvent::ClientConnected { client_id, pid } => {
                let internal_id = self
                    ...
                    .as_ref()
                    .unwrap()
                    .client_id_to_internal(client_id.clone());
            ...
            CompositorEvent::WindowCreated {
                ...
                let internal_client_id = self
                    ...
                    .as_ref()
                    .unwrap()
                    .client_id_to_internal(client_id);
            ...
            CompositorEvent::RedrawNeeded { window_id } => {
                self.pending_redraws.write().unwrap().push(WindowId {
                    id: window_id as u64,
                });
            }
            CompositorEvent::SurfaceCommitted {
                ...
                let buffer_id = if let Some(bid) = buffer_id {
                    ...
                    crate::wlog!(
                        crate::util::logging::FFI,
                        "FFI: SurfaceCommitted with no buffer_id"
                    );
                    return;
                };
                ...
                // -------------------------------------------------------
                {
                    ...
                    if let Some(data) = buffer_data {
                        if let Some(window_id) = target_window_id {
                            ...
                            crate::wlog!(
                                crate::util::logging::FFI,
                                "FFI: No window for surface {} in SurfaceCommitted",
                                surface_id
                            );
                        }
                    }
...
impl WawonaCore {
    ...
    pub fn inject_pointer_motion(&self, window_id: WindowId, x: f64, y: f64, timestamp_ms: u32) {
        ...
        state
            .seat
            .broadcast_pointer_motion(timestamp_ms, sx, sy, focused_client.as_ref());
    ...
    pub fn inject_pointer_frame(&self, _window_id: WindowId) {
        ...
        self.state
            ...
            .unwrap()
            .seat
            .broadcast_pointer_frame(None);
    }

    /// Inject pointer enter event
    pub fn inject_pointer_enter(&self, window_id: WindowId, x: f64, y: f64, _timestamp_ms: u32) {
        if !self.is_running() {
            return;
        }
    ...
    pub fn inject_key(&self, keycode: u32, state: KeyState, timestamp_ms: u32) {
        ...
        if mods_changed {
            ...
            state
                .seat
                .broadcast_modifiers(mod_serial, d, la, lo, g, focused_client.as_ref());
        }
    }
    ...
    pub fn inject_keyboard_enter(&self, window_id: WindowId, pressed_keys: Vec<u32>) {
        ...
        if let Some(sid) = surface_id {
            ...
            crate::wlog!(
                crate::util::logging::FFI,
                "Deferring keyboard enter for window {} (surface not yet mapped)",
                window_id.id
            );
            ...
            if let Some(surface) = state.get_surface(surface_id) {
                ...
                if let Some(res) = &surface.resource {
                    state
                        .seat
                        .touch
                        .broadcast_down(serial, timestamp_ms, res, touch_id, x, y);
                }
            }
        }
    ...
    pub fn inject_touch_up(&self, touch_id: i32, timestamp_ms: u32) -> Result<()> {
        ...
        state
            .seat
            .touch
            .broadcast_up(serial, timestamp_ms, touch_id, client.as_ref());
    ...
    pub fn inject_touch_motion(
        ...
        state
            .seat
            .touch
            .broadcast_motion(timestamp_ms, touch_id, x, y, client.as_ref());
    ...
    pub fn set_clipboard_text(&self, text: &str) {
        ...
        if let Some(comp) = compositor_guard.as_ref() {
            ...
            let mut state = self.state.write().unwrap();
            state.set_clipboard_source(
                &dh,
                Some(crate::core::state::SelectionSource::Host(text.to_string())),
            );
        }
    }
    ...
    pub fn get_render_scene(&self) -> RenderScene {
        ...
        for surface in flattened_scene {
            ...
            let texture_handle =
                if let Some(handle) = ffi_textures.get(&(surface.surface_id as u64)) {
                    *handle
    ...
    pub fn notify_window_frame_complete(&self, window_id: WindowId) {
        ...
        if let Some(surface_id) = surface_id {
            self.state
                .write()
                .unwrap()
                .flush_frame_callbacks(surface_id, None);
        }
    }
    ...
    pub fn disconnect_client(&self, client_id: ClientId) {
        ...
        self.pending_client_events
            .write()
            .unwrap()
            .push(ClientEvent::Disconnected { client_id });
    }
...
impl WawonaCore {
    ...
    pub fn text_input_get_content_type(&self) -> (u32, u32) {
        ...
        for (_id, instance) in &state.ext.text_input.instances {
            if instance.enabled {
                return (instance.content_type.hint, instance.content_type.purpose);
            }
        }
        (0, 0)
    }
    ...
    pub fn get_buffer_render_info(&self, texture: TextureHandle) -> BufferRenderInfo {
        ...
        let client_id = self
            ...
            .as_ref()
            .unwrap()
            .internal_to_client_id(texture.client_id.id);
